...
import { saveEducation } from "@/app/dashboard/profile/structured-actions";
import {
  emptyEducation,
  type EducationInput,
...
export default function EducationEditor({
  ...
  initialRows: EducationInput[];
}) {
  const [rows, setRows] = useState<EducationInput[]>(initialRows);
  const [status, setStatus] = useState<"idle" | "saved" | "error">("idle");
  const [message, setMessage] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  function update(id: string, patch: Partial<EducationInput>) {
    setRows((current) =>
  ...
  function addRow() {
    setRows((current) => [...current, emptyEducation(newId())]);
    setStatus("idle");
  }
  ...
  function save() {
    startTransition(async () => {
      const result = await saveEducation(rows);
      if (result.ok) {
        setStatus("saved");
            ...
            >
              <div className="grid gap-3 sm:grid-cols-2">
                <input
                  ...
                  onChange={(e) =>
                    update(row.id, { institution: e.target.value })
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    update(row.id, { fieldOfStudy: e.target.value })
                  }
              ...
              <textarea
                ...
                placeholder="Notable coursework, honors, or focus (optional)."
              />
